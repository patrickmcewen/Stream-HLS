...
class BitLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # STE using detach
        # NOTE: HAD TO REMOVE QUANTIZATION HERE BECAUSE IT CAUSED UNSUPPORTED INSTRUCTIONS IN MLIR
...
class OutputHead(nn.Module):
    ...
    Args:
        dim (int): The input dimension.
        vocab_size (int): The vocabulary size (output dimension).
    """
...
class Transformer(nn.Module):
    ...
    def forward(self, x: Tensor, *args, **kwargs) -> Tensor:
        ...
        for attn, ffn in zip(self.layers, self.ffn_layers):
            x = attn(x, x, x, is_causal=True, *args, **kwargs)
            x = self.norm(x + skip)
            x = ffn(x) + x
        return x
